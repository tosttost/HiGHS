...
#define HIGHS_CONST_TINY 10E-20
#define HIGHS_CONST_ZERO 10E-300
...
void solveHyper(const int Hsize, const int* Hlookup, const int* HpivotIndex,
                ...
                const int* Hindex, const double* Hvalue, Vector* rhs) {
  int RHScount = rhs->num_nz;
  int* RHSindex = &rhs->index[0];
  double* RHSarray = &rhs->value[0];
  ...
  cwork.assign(rhs->num_nz + 6400, 0);
  iwork.assign(rhs->num_nz * 4, 0);

  char* listMark = &cwork[0];
  int* listIndex = &iwork[0];
  int* listStack = &iwork[Hsize];
...
void QFactor::setup(int numCol_, int numRow_, const int* Astart_,
...
void QFactor::ftran(Vector& vector, double historical_density) const {
  ftranL(vector, historical_density);
  ftranU(vector, historical_density);
}

void QFactor::btran(Vector& vector, double historical_density) const {
  btranU(vector, historical_density);
  btranL(vector, historical_density);
}

void QFactor::update(Vector* aq, Vector* ep, int* iRow, int* hint) {
  // Special case
  // if (aq->next) {
  ...
  if (updateMethod == UPDATE_METHOD__FT) updateFT(aq, ep, *iRow);
  if (updateMethod == UPDATE_METHOD__PF) updatePF(aq, *iRow, hint);
  if (updateMethod == UPDATE_METHOD__MPF) updateMPF(aq, ep, *iRow, hint);
  if (updateMethod == UPDATE_METHOD__APF) updateAPF(aq, ep, *iRow);
}

void QFactor::buildSimple() {
  /**
...
void QFactor::buildHandleRankDeficiency() {
...
void QFactor::buildFinish() {
  ...
  int URstuffX = updateMethod == UPDATE_METHOD__FT ? 5 : 0;
  ...
  UtotalX = UcountX;
  if (updateMethod == UPDATE_METHOD__PF) UmeritX = numRow + UcountX * 4;
  if (updateMethod == UPDATE_METHOD__MPF) UmeritX = numRow + UcountX * 3;
...
void QFactor::ftranL(Vector& rhs, double historical_density) const {
  if (updateMethod == UPDATE_METHOD__APF) {
    rhs.sanitize();
    // rhs.pack();
    ftranAPF(rhs);
    rhs.sanitize();
  ...
  double current_density = 1.0 * rhs.num_nz / numRow;
  if (current_density > hyperCANCEL_ || historical_density > hyperFTRANL_) {
    // Alias to RHS
    int RHScount = 0;
    int* RHSindex = &rhs.index[0];
    double* RHSarray = &rhs.value[0];
...
void QFactor::btranL(Vector& rhs, double historical_density) const {
  double current_density = 1.0 * rhs.num_nz / numRow;
  if (current_density > hyperCANCEL_ || historical_density > hyperBTRANL_) {
    // Alias to RHS
    int RHScount = 0;
    int* RHSindex = &rhs.index[0];
    double* RHSarray = &rhs.value[0];
...
void QFactor::ftranU(Vector& rhs, double historical_density) const {
  // The update part
  if (updateMethod == UPDATE_METHOD__FT) {
    //    const double current_density = 1.0 * rhs.num_nz / numRow;
    ftranFT(rhs);
    rhs.sanitize();
    // rhs.pack();
  }
  if (updateMethod == UPDATE_METHOD__MPF) {
    ftranMPF(rhs);
    rhs.sanitize();
    // rhs.pack();
  }
  ...
  // The regular part
  const double current_density = 1.0 * rhs.num_nz / numRow;
  if (current_density > hyperCANCEL_ || historical_density > hyperFTRANU_) {
    ...
    if (report_ftran_upper_sparse) {
      const double final_density = 1.0 * rhs.num_nz / numRow;
      printf(
  ...
  if (updateMethod == UPDATE_METHOD__PF) {
    ftranPF(rhs);
    rhs.sanitize();
    // rhs.pack();
  }
...
void QFactor::btranU(Vector& rhs, double historical_density) const {
  if (updateMethod == UPDATE_METHOD__PF) {
    btranPF(rhs);
  }
  ...
  // The regular part
  double current_density = 1.0 * rhs.num_nz / numRow;
  if (current_density > hyperCANCEL_ || historical_density > hyperBTRANU_) {
    // Alias to non constant
    double RHS_syntheticTick = 0;
    int RHScount = 0;
    int* RHSindex = &rhs.index[0];
...
void QFactor::ftranFT(Vector& vector) const {
  // Alias to PF buffer
  ...
  // Alias to non constant
  int RHScount = vector.num_nz;
  int* RHSindex = &vector.index[0];
  double* RHSarray = &vector.value[0];
...
void QFactor::btranFT(Vector& vector) const {
  // Alias to PF buffer
  const int PFpivotCount = PFpivotIndex.size();
...
void QFactor::ftranPF(Vector& vector) const {
  // Alias to PF buffer
  const int PFpivotCount = PFpivotIndex.size();
...
void QFactor::btranPF(Vector& vector) const {
  // Alias to PF buffer
  const int PFpivotCount = PFpivotIndex.size();
  const int* PFpivotIndex = &this->PFpivotIndex[0];
  const double* PFpivotValue = &this->PFpivotValue[0];
...
void QFactor::ftranMPF(Vector& vector) const {
  // Alias to non constant
  int RHScount = vector.num_nz;
  int* RHSindex = &vector.index[0];
  double* RHSarray = &vector.value[0];
...
void QFactor::btranMPF(Vector& vector) const {
  // Alias to non constant
  int RHScount = vector.num_nz;
  int* RHSindex = &vector.index[0];
  double* RHSarray = &vector.value[0];
...
void QFactor::ftranAPF(Vector& vector) const {
  // Alias to non constant
  int RHScount = vector.num_nz;
  int* RHSindex = &vector.index[0];
  double* RHSarray = &vector.value[0];
...
void QFactor::btranAPF(Vector& vector) const {
  // Alias to non constant
  int RHScount = vector.num_nz;
  int* RHSindex = &vector.index[0];
  double* RHSarray = &vector.value[0];
...
/*
void QFactor::updateCFT(Vector* aq, Vector* ep, int* iRow
                        //, int* hint
) {
  // 
  ...
  int numUpdate = 0;
  for (Vector* vec = aq; vec != 0; vec = vec->next) numUpdate++;

  Vector** aqWork = new Vector*[numUpdate];
  Vector** epWork = new Vector*[numUpdate];
...
void QFactor::updateFT(Vector* aq, Vector* ep, int iRow
                       //, int* hint
) {
  // Store pivot
  int pLogic = UpivotLookup[iRow];
  double pivot = UpivotValue[pLogic];
  double alpha = aq->value[iRow];
  ...
  // Store column to U
  Ustart.push_back(Uindex.size());
  for (int i = 0; i < aq->num_nz; i++)
    if (aq->index[i] != iRow) {
      Uindex.push_back(aq->index[i]);
      Uvalue.push_back(aq->value[aq->index[i]]);
    }
  ...
  for (int i = 0; i < ep->num_nz; i++) {
    if (ep->index[i] != iRow) {
      PFindex.push_back(ep->index[i]);
      PFvalue.push_back(-ep->value[ep->index[i]] * pivot);
    }
  }
  UtotalX += PFindex.size() - PFstart.back();
...
void QFactor::updatePF(Vector* aq, int iRow, int* hint) {
  // Check space
  const int columnCount = aq->num_nz;
  const int* columnIndex = &aq->index[0];
  const double* columnArray = &aq->value[0];

  // Copy the pivotal column
  for (int i = 0; i < columnCount; i++) {
    int index = columnIndex[i];
    double value = columnArray[index];
    if (index != iRow) {
      PFindex.push_back(index);
  ...
  // Save pivot
  PFpivotIndex.push_back(iRow);
  PFpivotValue.push_back(aq->value[iRow]);
  PFstart.push_back(PFindex.size());
...
void QFactor::updateMPF(Vector* aq, Vector* ep, int iRow, int* hint) {
  // Store elements
  for (int i = 0; i < aq->num_nz; i++) {
    PFindex.push_back(aq->index[i]);
    PFvalue.push_back(aq->value[aq->index[i]]);
  }
  int pLogic = UpivotLookup[iRow];
  ...
  for (int i = 0; i < ep->num_nz; i++) {
    PFindex.push_back(ep->index[i]);
    PFvalue.push_back(ep->value[ep->index[i]]);
  }
  PFstart.push_back(PFindex.size());
  ...
  // Refactor or not
  UtotalX += aq->num_nz + ep->num_nz;
  if (UtotalX > UmeritX) *hint = 1;
}

void QFactor::updateAPF(Vector* aq, Vector* ep, int iRow
                        //, int* hint
) {
  // Store elements
  for (int i = 0; i < aq->num_nz; i++) {
