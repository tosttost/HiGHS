...
void HighsImplications::separateImpliedBounds(
  ...
  HighsInt maxEntries =
      100000 + mipsolver.mipdata_->numCliqueEntriesAfterPresolve +
      4 * (//mipsolver.mipdata_->numCliqueEntriesAfterFirstPresolve +
           mipsolver.numNonzero());
  // printf("numEntries: %d, maxEntries: %d\n",
  //       mipsolver.mipdata_->cliquetable.getNumEntries(), maxEntries);
  if (!mipsolver.submip &&
      mipsolver.mipdata_->cliquetable.getNumEntries() <= maxEntries) {
    for (std::pair<HighsInt, double> fracint :
      ...
      if (mipsolver.mipdata_->cliquetable.getNumEntries() > maxEntries) break;
    }
  }
