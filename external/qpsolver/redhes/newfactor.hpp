...
void resize() {
   ...
   for (unsigned int i=0; i<current_k_max; i++) {
      for (unsigned int j=0; j<current_k_max; j++) {
...
public:
   NewCholeskyFactor(Runtime& rt, Basis& basis, Nullspace& ns) : runtime(rt), nullspace(ns) {
      uptodate = false;
      printf("computed size: %u, needed size: %u\n", min((unsigned int)ceil(rt.instance.num_var / 16.0), 1000u), basis.getnuminactive());
      current_k_max = max(min((unsigned int)ceil(rt.instance.num_var / 16.0), 1000u), basis.getnuminactive());
...
public:
   ...
   void eliminate(std::vector<double>& m, unsigned int i, unsigned int j, unsigned int kmax, unsigned int currentk) {
      // i = col, j = row
      if (m[j * kmax +i] == 0.0) {
         return;
      }
   ...
   void reduce(NullspaceReductionResult& nrr) {
      ...
      for (unsigned int row=p; row<current_k-1; row++) {
         for (unsigned int i=0; i<current_k; i++) {
      ...
      for (unsigned int row=0; row<current_k; row++) {
         double p_entry = L[row * current_k_max + p];
         for (unsigned int col=p; col<current_k-1; col++) {
      ...
      for (unsigned int i=0; i<current_k-1; i++) {
   ...
   void report(std::string name = "") {
      printf("%s\n", name.c_str());
      for (unsigned int i=0; i<current_k; i++) {
         for (unsigned int j=0; j<current_k; j++) {
